...
function computerHTML(tab){
  ...
  if(tab==="cctv"){
    return `
      <h3>CCTV SYSTEM</h3>
      <div class="cctv-controls-v36">
        <button data-cctv="full">전체</button>
        <button data-cctv="1">CAM 1</button>
        <button data-cctv="2">CAM 2</button>
        <button data-cctv="3">CAM 3</button>
        <button data-cctv="4">CAM 4</button>
      </div>
      <canvas id="cctvCanvasV36" width="560" height="320"></canvas>
      <p id="cctvStatusV36">신호 정상 · 전력 ${Math.round(power)}%</p>
    `;
  }
...
function openComputer(){
  ...
  if(!hacked)setTimeout(()=>renderCCTVViewV36("full"),0);
}
...
document.querySelector(".computer-tabs").onclick=e=>{
  ...
  if(!hacked && tab==="cctv")setTimeout(()=>renderCCTVViewV36("full"),0);
};
...
function payHackerV36FromComputer(){
  ioClient.emit("v32-pay-hacker",{},r=>{
    ...
    $("computerContent").innerHTML=computerHTML("cctv");
    setTimeout(()=>renderCCTVViewV36("full"),0);
...
function renderCCTVViewV36(camera="full"){
  if(hacked)return;
  const c=$("cctvCanvasV36");
  const src=$("bunkerCanvas");
  if(!c||!src)return;
  const ctx=c.getContext("2d");
  const W=c.width,H=c.height;
  ctx.fillStyle="#07100b";ctx.fillRect(0,0,W,H);

  const sw=src.width||1000, sh=src.height||800;
  let sx=0,sy=0,cw=sw,ch=sh;
  if(camera==="1"){cw=sw/2;ch=sh/2;sx=0;sy=0}
  if(camera==="2"){cw=sw/2;ch=sh/2;sx=sw/2;sy=0}
  if(camera==="3"){cw=sw/2;ch=sh/2;sx=0;sy=sh/2}
  if(camera==="4"){cw=sw/2;ch=sh/2;sx=sw/2;sy=sh/2}

  try{ctx.drawImage(src,sx,sy,cw,ch,0,0,W,H)}catch(_){}

  // CCTV scan lines / low power noise
  ctx.save();
  ctx.globalAlpha=.18;
  ctx.fillStyle="#8dffb2";
  for(let y=0;y<H;y+=5)ctx.fillRect(0,y,W,1);
  ctx.restore();

  if(power<35){
    ctx.save();
    ctx.globalAlpha=.26+(35-power)/100;
    for(let i=0;i<90;i++){
      const x=Math.random()*W,y=Math.random()*H,w=4+Math.random()*35;
      ctx.fillStyle=Math.random()>.5?"#fff":"#4cff83";
  ...
  if(power<=0){
    ...
    ctx.fillStyle="#f66";ctx.font="bold 24px sans-serif";ctx.textAlign="center";
    ctx.fillText("NO POWER",W/2,H/2);
  }

  ctx.fillStyle="#82ffa4";ctx.font="bold 14px monospace";ctx.textAlign="left";
  ctx.fillText(`CCTV ${camera==="full"?"OVERVIEW":"CAM "+camera}`,12,22);
  ctx.fillText(`POWER ${Math.round(power)}%`,12,42);

  const st=$("cctvStatusV36");
  if(st)st.textContent=power<=0?"전력 없음":power<35?`저전력 · 영상 노이즈 · ${Math.round(power)}%`:`신호 정상 · 전력 ${Math.round(power)}%`;
}
...
ioClient.on("computer-hacked",()=>{
  hacked=true;firewall=0;renderV32SystemHUD();
  if(!$("computerPanel")?.classList.contains("hidden")){

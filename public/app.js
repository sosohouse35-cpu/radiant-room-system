...
const BUNKER_OBJECTS = [
  ...
  {id:"radioStation",label:"라디오",x:465,y:112,w:62,h:48,solid:true,kind:"radioStation"},
  ...
  /* 샤워실은 벽 없는 개방형 공간 */
  {id:"showerRoom",label:"샤워실",x:650,y:190,w:132,h:190,solid:false,kind:"room"}
];
...
function renderRadioPanelV372(){
  ...
  if(!s.hasRadio){
    ...
    event.textContent="현재 수신 중인 이벤트가 없습니다.";
    actions.innerHTML="";
  }
...
function openRadioPanelV372(){
  $("radioPanelV372").classList.remove("hidden");
  refreshRadioStateV372();
}

$("radioCloseV372").onclick=()=> $("radioPanelV372").classList.add("hidden");

$("radioTuneV372").onclick=()=>{
  ioClient.emit("v372-radio-tune",r=>{
    if(!r?.ok){
      if(r?.cooldown){
        toast(`새 신호까지 약 ${Math.ceil((r.remaining||0)/1000)}초`);
      }else{
...
$("radioEventActionsV372").onclick=e=>{

...
function draw(){
  ...
  furn[floor].forEach(o=>{
    if(v34DrawScavengeFurniture(ctx,o,camX,camY))return;
...
const V34_ASSET_BASE="/assets/v34/";
const V34_IMAGES={};
const V34_ASSET_PATHS={
  metalFloor:"tiles/bunker_metal_floor.png",
  bed:"furniture/bed_user.png",
  computer:"furniture/computer.png",
  locker:"furniture/locker.png",
  storage:"furniture/shelf.png",
  medical:"items/medkit.png",
  blueprint:"items/blueprint.png",
  power:"furniture/generator.png",
  water:"items/water.png",
  vent:"furniture/vent.png",
  ventVertical:"furniture/vent.png",
  sink:"furniture/sink.png",
  fridge:"furniture/fridge.png",
  toilet:"furniture/toilet.png",
  trash:"furniture/trash.png",
  cabinet:"furniture/cabinet.png",
  shelf:"furniture/shelf.png",
  workbench:"furniture/workbench.png",
  stove:"furniture/stove.png",
  washing_machine:"furniture/washing_machine.png"
};
for(const [key,rel] of Object.entries(V34_ASSET_PATHS)){
  const im=new Image(); im.src=V34_ASSET_BASE+rel; V34_IMAGES[key]=im;
}
function v34DrawSprite(ctx,key,x,y,w,h,alpha=1){
  const im=V34_IMAGES[key];
  if(!im||!im.complete||!im.naturalWidth)return false;
  ctx.save();ctx.globalAlpha=alpha;ctx.drawImage(im,x,y,w,h);ctx.restore();return true;
}
function v34FurnitureKey(label){
  return ({
    "싱크대":"sink","욕조":"toilet","침대":"bed","책상":"computer",
    "책장":"shelf","선반":"shelf","상자":"cabinet","작업대":"workbench",
    "조리대":"workbench","탁자":"cabinet","식탁":"cabinet"
  })[label]||null;
}
function v34DrawScavengeFurniture(ctx,o,camX,camY){
  const key=v34FurnitureKey(o.label),x=o.x-camX,y=o.y-camY;
  return !!(key&&v34DrawSprite(ctx,key,x,y,o.w,o.h));
}

const BUNKER_OBJECTS = [
  {id:"weapons",label:"무기 보관함",x:118,y:92,w:74,h:130,solid:true,kind:"locker"},
  {id:"bed",label:"침대",x:118,y:238,w:165,h:92,solid:true,kind:"bed"},
  {id:"computer",label:"컴퓨터",x:118,y:350,w:112,h:62,solid:true,kind:"computer"},
  {id:"beans",label:"통조림",x:118,y:430,w:118,h:82,solid:true,kind:"storage"},
  {id:"medkit",label:"메디킷",x:760,y:640,w:118,h:60,solid:true,kind:"medical"},
  {id:"blueprints",label:"블루프린트",x:270,y:710,w:150,h:52,solid:true,kind:"blueprint"},
  {id:"power",label:"전력 공급기",x:830,y:365,w:102,h:76,solid:true,kind:"power"},
  {id:"water",label:"물",x:862,y:468,w:70,h:120,solid:true,kind:"water"},
...
function drawBunkerInterior(){
  ...
  // VERSION 34: 철판 이미지 타일 벙커 바닥
  const floorImg=V34_IMAGES.metalFloor;
  if(floorImg?.complete&&floorImg.naturalWidth){
    const pattern=bctx.createPattern(floorImg,"repeat");
    bctx.save();bctx.translate(100-camX,60-camY);
    bctx.fillStyle=pattern;bctx.fillRect(0,0,850,720);bctx.restore();
  }else{
    bctx.fillStyle="#596168";
  ...
  BUNKER_OBJECTS.filter(o=>o.id!=="showerRoom").forEach(o=>{
    ...
    const spriteKey={
      locker:"locker",bed:"bed",computer:"computer",storage:"storage",
      medical:"medical",blueprint:"blueprint",power:"power",water:"water",
      vent:"vent",ventVertical:"ventVertical"
    }[o.kind];

    if(spriteKey&&v34DrawSprite(bctx,spriteKey,x,y,o.w,o.h)){
      // 이미지 스프라이트 사용
...
function computerHTML(tab){
  ...
  if(tab==="security"){
    return `
      ...
      <p>Firewall: <b>${Math.round(firewall)}%</b></p>
...
function startGeneratorTimingV32(){
  ioClient.emit("v32-generator-action",{action:"timing"},r=>{
    if(!r?.ok){
      if(r?.needsBattery)toast("⚠ 정전 상태: 배터리 1개로 먼저 재가동해야 합니다.");
      else toast(r?.message||"발전기 오류");
      return;
    }
    const bar=$("generatorCursor"), target=$("generatorTarget");
    let x=0,dir=1,active=true;
    const targetPct=r.target*100;
    if(target){target.style.left=`${targetPct-7}%`;target.style.width="14%";}
    function anim(){
      if(!active)return;
      x+=dir*1.7;if(x>=100){x=100;dir=-1}if(x<=0){x=0;dir=1}
      if(bar)bar.style.left=`${x}%`;
      requestAnimationFrame(anim);
    }
    requestAnimationFrame(anim);
    $("generatorHit").onclick=()=>{
      if(!active)return;active=false;
      ioClient.emit("v32-generator-submit",{value:x/100},res=>{
        if(res?.ok){power=res.state.power;blackout=res.state.blackout;toast(`전력 +${res.gain}%`);renderV32SystemHUD();}
      });
    };
  });
}

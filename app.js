...
function wallRect(x,y,w,h){for(let r=y;r<y+h;r++)for(let c=x;c<x+w;c++)if(r>=0&&r<ROWS&&c>=0&&c<COLS)grid[r][c]=1}
function door(x,y,w=1,h=1){for(let r=y;r<y+h;r++)for(let c=x;c<x+w;c++)if(grid[r])grid[r][c]=0}
function buildMap(){
 wallRect(0,0,COLS,1);wallRect(0,ROWS-1,COLS,1);wallRect(0,0,1,ROWS);wallRect(COLS-1,0,1,ROWS);
 ...
 wallRect(8,3,1,13);wallRect(15,3,1,13);wallRect(33,3,1,13);wallRect(48,3,1,13);
 door(5,16,2);door(22,16,2);door(35,16,2);door(50,16,2);door(19,24,1,3);door(40,24,1,3);
 door(8,9,1,2);door(15,10,1,2);door(33,10,1,2);door(48,10,1,2);
 /* furniture */
 wallRect(5,7,4,2);wallRect(18,6,5,2);wallRect(29,7,3,4);wallRect(44,7,5,2);
 wallRect(5,22,5,2);wallRect(12,28,4,3);wallRect(25,22,7,2);wallRect(47,24,5,4);
}
buildMap();
...
socket.on("game-started",data=>{
 showScreen("game");players={};data.players.forEach(p=>players[p.id]={...p});me={...players[myId]};items=data.items;endsAt=data.endsAt;

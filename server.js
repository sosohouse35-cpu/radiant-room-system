...
function v3716BreachExteriorThreats(room){
 ...
 s.on("v372-radio-tune",(cb=()=>{})=>{
  ...
  if(r.radioState.interference)return cb({ok:false,message:"강한 전파 방해로 채널을 바꿀 수 없습니다."});

  if(r.radioState.currentEvent){
    return cb({ok:true,state:radioPublicStateV372(r)});
  }

  if(Date.now()<r.radioState.nextEventAt){
    return cb({
      ok:false,
      cooldown:true,
      remaining:r.radioState.nextEventAt-Date.now(),
      message:"아직 새로운 신호가 잡히지 않습니다."
    });
  }

  radioCreateEventV372(r);
...
setInterval(()=>{
  ...
  for(const r of rooms.values()){
    ...
    // Notify when a signal is ready, but don't force-open the event.
    if((r.bunkerStock.radio||0)>0 && !rs.currentEvent && !rs.interference && now>=rs.nextEventAt){
      io.to(r.code).emit("v372-radio-ready",{ready:true});
    }
  }
},3000);
